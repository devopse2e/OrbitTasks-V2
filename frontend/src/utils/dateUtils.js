...
import {
  ...
  isEqual,
  parseISO,
  set,  // NEW: Added for preserving time in recurring instances
} from 'date-fns';
...
export function generateRecurringInstances(todo, rangeStart, rangeEnd, timeZone) {
  ...
  const addInterval = (date, pattern, intervalCount) => {
    ...
    let newZoned = toZonedTime(newUtcDate, timeZone); // Back to zoned
    // NEW: Preserve original time (e.g., 1:00 PM)
    const originalHours = date.getHours();
    const originalMinutes = date.getMinutes();
    newZoned = set(newZoned, { hours: originalHours, minutes: originalMinutes });
    return newZoned;
  };
